		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
                ...
                if ($user_logged_obj->isFriend($added_by)) {
						...
						// $post_query = mysqli_query($this->con, "SELECT body FROM posts WHERE id='$id'");
						// $post_row = mysqli_fetch_array($user_details_query);
						// $post_body = $post_row['body'];
                ...
                <script>
					...
					$(document).ready(function() {
                        $('#post<?php echo $id; ?>').on('click', function() {
								location.replace("post.php?id=<?php echo $id; ?>&edit=yes");
                        });
                    });
                    ...
                    <?php
						...
						if ($deleted == 'no')
							...
							$str .= "
                                    ...
                                    <div class='newsfeedPostOptions'>
									Comments($comments_check_num)&nbsp;&nbsp;&nbsp;
									<iframe src='like.php?post_id=$id' scrolling='no' style='background-color: transparent;'></iframe>
								    </div>
    ...
    public function getSinglePost($post_id, $edit) {
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			if($row['user_to'] == "none") {
				...
				$user_logged_obj = new User($this->con, $userLoggedIn);
				if($user_logged_obj->isFriend($added_by) || $userLoggedIn == 'admin') {
					if($userLoggedIn == $added_by) {
						$delete_button = "<button class='delete_button' id='delete_post$id' style='background-color: transparent; font-weight: bold; border: none;'><i class='fa-regular fa-trash-can'></i></button>";
				...
				<?php
					...
					if ($edit == "yes")
						$str .= "<textarea id='edit_body' style='height: 95px; width: 85%;'>$body</textarea>
						$save_button";
					else 
						$str .= $body;
					...
					$(document).ready(function() {
						$('#edit_post<?php echo $id; ?>').on('click', function() {
							if ('<?php echo $edit; ?>' == 'yes')
								location.replace("post.php?id=<?php echo $id; ?>&edit=no");
							else
								location.replace("post.php?id=<?php echo $id; ?>&edit=yes");
						});
					});
